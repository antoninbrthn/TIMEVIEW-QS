...
class BetaSinDataset(BaseDataset):
    def __init__(self, n_samples=100, n_timesteps=20):
        super().__init__(n_samples=n_samples, n_timesteps=n_timesteps)
        n_samples_per_dim = int(n_samples ** (1 / 3))
        n_samples = n_samples_per_dim**2
        alphas = np.linspace(1.0, 4.0, n_samples_per_dim)
        betas = np.linspace(1.0, 4.0, n_samples_per_dim)
        gammas = np.linspace(-2, 2, n_samples_per_dim)

        grid = np.meshgrid(alphas, betas, gammas)

        # stack along the last axis and then reshape into 3 columns
        cart_prod = np.stack(grid, axis=-1).reshape(-1, 3)
        ...
        self.ys = [
            np.array(
                [
                    beta.pdf(t, alpha, betap) * np.sin(np.pi * gammap * t)
                    for t in np.linspace(0, 1, n_timesteps)
                ]
            )
            for alpha, betap, gammap in zip(
                self.X["alpha"], self.X["beta"], self.X["gamma"]
            )
        ]
    ...
    def get_feature_ranges(self):
        return {"alpha": (1.0, 4.0), "beta": (1.0, 4.0), "gamma": (-2.0, 2.0)}
